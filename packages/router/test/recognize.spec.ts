...
describe('recognize', () => {
  ...
  describe('multiple matching routes', () => {
    it('should return an array of all matching routes', () => {
      recognize(
        RootComponent,
        [
          {path: 'nomatch', component: ComponentA},
          {path: 'a', component: ComponentA},
          {path: ':b', component: ComponentB},
          {path: 'anon', component: ComponentA},
...
function checkRecognize(config: Routes, url: string, callback: any): void {
  recognize(RootComponent, config, tree(url), url)
    .subscribe((x)=>callback(x.length > 0 ? x[0] : null), e => { throw e; });
}
